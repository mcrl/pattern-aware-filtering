...
def naming(red, green, k):
    return f"pld-r{red}-g{green}-ptf-{k}"
...
def file_routine(fpath, src_dir):
    ...
    for red in reds:
        for green in greens:
            for kidx, k in enumerate(ks):
                names.append(naming(red, green, k))
                thresholds.append((red, green, kidx))
    ...
    with gzip.open(src_fpath, 'rt', encoding='utf-8') as fin:
        ...
        for line in fin:
            ...
            lines = text.split("\n")
            for i, (red, green, kidx) in enumerate(thresholds):
                cp_lines = pld_extractor(lines, count, red=red, green=green)
                # Apply pattern-based remover
                compiled = punc_patterns[kidx]
                papm_lines = ptf_extractor(cp_lines, compiled)
                
                if papm_lines:
                    data = {"text": "\n".join(papm_lines)}
                    docs[i].append(json.dumps(data, ensure_ascii=False))
